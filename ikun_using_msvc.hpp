// ikun库自动包含命名空间模块 ikun_using_msvc.hpp
// 请在代码#include指令最下面包含本头文件
// 注: 由于命名空间std可能导致某些冲突, 故本文件只添加常用的std成员
// 本文件使用的是MSVC STL标准库, 请保证编译器为Clang(clang-cl模式)或MSVC
...
#ifndef IKUN_USING_MSVC_HPP
#define IKUN_USING_MSVC_HPP
