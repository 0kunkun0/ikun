...
compilerargs_on_windows = {
    1: "g++ -std=c++23 -O3 -lstdc++exp ikun_core.cpp -o ikun.exe -static -lstdc++exp",
    2: "clang++ -std=c++23 -O3 -lstdc++exp -stdlib=libc++ ikun_core.cpp -o ikun.exe -lstdc++exp",
    3: "cl /EHsc /nologo /std:c++latest /O2 /MT /Zc:__cplusplus ikun_core.cpp /Fe:ikun.exe",
    4: "g++ -std=c++23 -O3 -lstdc++exp ikun_error_analyzer.cpp -o ikun_error_analyzer.exe -static -lstdc++exp",
    5: "clang++ -std=c++23 -O3 -lstdc++exp -stdlib=libc++ ikun_error_analyzer.cpp -o ikun_error_analyzer.exe -lstdc++exp",
    6: "cl /EHsc /nologo /std:c++latest /O2 /MT /Zc:__cplusplus ikun_error_analyzer.cpp /Fe:ikun_error_analyzer.exe"
}
compilerargs_on_linux = {
    1: "g++ -std=c++2b -O3 ikun_core.cpp -o ikun -static -lstdc++exp",
    2: "clang++ -std=c++2b -O3 -stdlib=libc++ ikun_core.cpp -o ikun -static -lstdc++exp",
    3: "g++ -std=c++2b -O3 ikun_error_analyzer.cpp -o ikun_error_analyzer -static -lstdc++exp",
    4: "clang++ -std=c++2b -O3 -stdlib=libc++ ikun_error_analyzer.cpp -o ikun_error_analyzer -static -lstdc++exp"
}
...
def build_core():
    ...
    if compiler in compilerargs_on_windows or compiler in compilerargs_on_linux:
        if compiler == 3 and not system_status.startswith("Windows"):
            ...
            return 1
        if system_status.startswith("Windows"):
            print("正在编译ikun_core.cpp...")
            system(compilerargs_on_windows[compiler])
            print("正在编译ikun_error_analyzer.cpp...")
