## ikun库概述
- ikun库是一个跨平台的C++编程工具库, 支持Windows, Linux和macOS系统, 遵循GPL 3.0许可证, 此版本是7.1.2 RC版, 预计2026年2月发布正式版
 - 提供了丰富的功能, 包括但不限于:
 文件操作(files.hpp)
 数学运算(maths.hpp)
 随机数生成(random.hpp)
...
```
---
## ikun库错误分析(可直接用于AI或发送给开发者, 详见ikun_error_analyzer.cpp):
...
- ikun_hpd: (高精度整数库错误, `high_precision_digit.hpp`)
 ...
 - 007: 在使用`high_precision_digit::bigint::to_long_long()`函数时, **整数超出int64_t范围**(Out of Range)
...
- ikun_file: (文件相关错误, `files.hpp`)
 ...
 - 003:004: `files::filedir()`函数**发生未知错误(错误由filesystem库抛出)**(Runtime Error)
...
- ikun_random: (随机数相关错误, `random.hpp`)
 - 001: 在使用random_::Random::random_int()或random_double()时, min大于max (Invalid Argument)
 - 002: 在使用random_::Random::random_string()函数生成随机字符串时, 提供的长度为负数(Invalid Argument)
---
### files.hpp:
```cpp
namespace files
